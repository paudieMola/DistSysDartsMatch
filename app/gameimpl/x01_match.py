...
class X01Match(MatchManager, MatchVisitTemplate):
    ...
    def __init__(self):
        ...
        self.averages = []  # single dart avg x3 for 3 dart avg
        self.first9 = []  # avg for first 9 darts
    ...
    def post_init(self):
        for i in range(0, len(self.match.players)):
            self.scores.append(STARTING_TOTAL)  # might want to
    ...
    def check_winning_condition(self, player_index, visit):
        """
        returns 1,2, or 3 for when a dart closes the game/leg
        (i.e. finishing double) or a 0 when not closed out
    ...
    def record_statistics(self, player_index, visit, result):
        """
        Store stats both for in-memory immediate use and on disk for
        later recall
        """

        if result is not 0:
            visit.remove_trailing_darts(result)
            # a double finished game, ignore any subsequent darts

        self.match.visits[player_index].append(visit)

        # calculate first 9 if and only if this is 3rd visit.
        if len(self.match.visits[player_index]) == 3:
            self.first9[player_index] = (STARTING_TOTAL - self.scores[player_index])/3

        # Calculate single-dart avg taking account of a double
        # being hit with dart 1 or 2 when checking out
